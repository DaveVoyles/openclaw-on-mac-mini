...
async def read_inbox(provider: str = "gmail", count: int = 10) -> str:
    ...
    try:
        messages = await asyncio.to_thread(
            _imap_read_inbox, user, password, imap_host, count
        )
...
async def search_emails(query: str, provider: str = "gmail") -> str:
    ...
    try:
        messages = await asyncio.to_thread(
            _imap_search, user, password, imap_host, query, provider
        )
    except imaplib.IMAP4.error as e:
...
async def send_email(
    ...
    try:
        await asyncio.to_thread(
            _smtp_send, user, password, smtp_host, smtp_port, to, subject, body
        )
