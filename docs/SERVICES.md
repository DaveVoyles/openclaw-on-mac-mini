...
| **Ollama (Local)**     | https://ollama.com             | Self-hosted LLM runtime (qwen3:8b)       | Low-latency local model for simple conversational turns; keeps costs near zero for lightweight tasks. Runs on M4 (5.2 GB). Switched from gemma4:e4b due to memory constraints on 16 GB Mac mini.    | `OLLAMA_URL`, `OLLAMA_MODEL`     |
