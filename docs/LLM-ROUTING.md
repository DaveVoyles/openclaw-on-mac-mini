...
| W11-F | **`/feedback summary` Discord command** — reads `data/feedback.jsonl`, computes thumbs-up rate per provider + per skill, and posts a formatted embed; allows server owners to see which configurations users rate highest without leaving Discord | Low | `src/discord_commands/` | W11-B | ✅ Done |
...
| W12-C | **`/admin reload-routing`** Discord command — reloads `model_routing_policy.py` via `importlib.reload()` without restarting the bot; useful for tuning routing profiles or regex patterns in production without a deploy cycle; restrict to bot owner | Low | `src/discord_commands/` | — | ✅ Done |
