...
| 25 | **Per-provider breakdown in `token_usage_summary()`** — the current `_cumulative_tokens` dict aggregates all providers into a single `{"input": N, "output": N}`; extend it to `{"total": {"input": N, "output": N}, "by_provider": {"openai": {...}, "anthropic": {...}, "copilot": {...}}}` for granular cost attribution per provider | Low | `src/llm/providers.py` | 8 | 🔄 In Progress — r25-token-breakdown |
