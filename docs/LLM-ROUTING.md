...
| 14 | **Routing telemetry / audit log** — after each `call_provider()` call, append a JSON line to `data/routing_audit.jsonl` with timestamp, query_hash, provider, model, latency_ms, token counts; expose aggregated summary at `/metrics` | Medium | `src/llm/providers.py`, `src/discord_web.py` | 13 | 🔄 In Progress — r14-telemetry |
...
| 22 | **Thread-safe `_last_usage` for async concurrency** — the current module-level `_last_usage` dict is a race condition under concurrent `call_provider()` calls; replace it with `contextvars.ContextVar` so each async task carries its own usage snapshot without clobbering neighbours | Medium | `src/llm/providers.py` | 19 | ⬜ Open |
