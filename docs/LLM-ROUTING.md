...
| 44 | **Expose `token_usage_summary()` via HTTP health endpoint** — `token_usage_summary()` now returns `{"total": {...}, "by_provider": {...}}`; surface this in the existing web health handler (e.g. `/health/llm`) so operators can inspect live token-burn rates per provider without needing direct Python access; the handler already calls provider introspection functions, so this is a small additive change; include a `reset_token_usage()` call on startup so metrics reflect the current process lifetime only | Low | `src/discord_web.py` (or health handler module), `src/llm/providers.py` | 25 | 📋 Proposed |
