...
async def test_github_prs_success_with_prs():
    ...
    with patch("config.cfg", cfg_mock), patch("httpx.AsyncClient") as mock_client_cls:
...
async def test_github_prs_generic_exception():
    ...
    with patch("config.cfg", cfg_mock), patch("httpx.AsyncClient", side_effect=Exception("network error")):
        await cog.github_prs.callback(cog, inter, repo="owner/repo")
...
async def test_github_issues_generic_exception():
    ...
    with patch("config.cfg", cfg_mock), patch("httpx.AsyncClient", side_effect=Exception("boom")):
        await cog.github_issues.callback(cog, inter, repo="owner/repo", label="")
...
async def test_github_watch_success(tmp_path, monkeypatch):
    ...
    with (
        patch("cogs.github_cog._load_watches", AsyncMock(return_value={})),
...
async def test_github_watch_already_watching():
    ...
    with (
        patch("cogs.github_cog._load_watches", AsyncMock(return_value=existing)),
...
async def test_monitor_task_with_watches():
    ...
    with (
        patch("cogs.github_cog._load_watches", AsyncMock(return_value=watches)),
        patch("cogs.github_cog._save_watches", AsyncMock()),
