...
class TestVectorStoreReexports:
    ...
    def test_memory_exports(self):
        ...
        for sym in [
            add_conversation_summary, add_memory, add_memory_deduped,
            add_research_report, clear_scoped_memory, get_scoped_memory_summary,
            recall, recall_for_context,
        ]:
            assert callable(sym)
    ...
    def test_scope_exports(self):
        ...
        for sym in [
            _allow_fallback_result, _combine_scope_where,
            _extract_explicit_recall_domains, _infer_recall_domains,
            _inject_scope_metadata, _is_legacy_metadata,
            _normalize_scope_id, _resolve_scope,
        ]:
            assert callable(sym)
