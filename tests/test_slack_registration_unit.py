...
Counts at time of last update (src/slack_bot.py):
  ...
  @app.command: 50 (/arr, /batch, /brief, /calendar, /chat, /clawbox, /clawchan, /clear,
                     /contacts, /copilot ×5, /digest, /drive, /email, /files, /filesearch,
                     /h, /health, /help, /hermes, /host, /inbox, /incident, /metrics,
                     /morning, /mypins, /mystats, /nas, /nickname, /plex, /q, /request,
                     /research, /resume, /schedule, /sessions, /simple, /status,
...
EXPECTED_COMMAND_COUNT = 50  # includes /arr, /copilot ×5 subcommand handlers, /h, /hermes, /host, /incident, /morning, /nas, /plex, /q, /request, /resume, /sessions, /status, /tailscale, /upcoming, /uptime, /wake, /watching, etc.
EXPECTED_ACTION_COUNT = 9    # includes incident_action_run
EXPECTED_TOTAL_COUNT = EXPECTED_EVENT_COUNT + EXPECTED_COMMAND_COUNT + EXPECTED_ACTION_COUNT  # 64
...
class TestSlackHandlerRegistration:
    ...
    def test_slack_registration_ast_total_count(self):
        """Total @app.* registrations must equal the expected baseline (64).
